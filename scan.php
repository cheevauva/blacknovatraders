...
if ($targetinfo['sector'] != $playerinfo['sector']) {
    ...
    if ($playerinfo['turns'] < 1) {
        ...
        if ($roll > $success) {
            ...
            $btyamount = 0;
            $hasbounty = db()->fetch("SELECT SUM(amount) AS btytotal FROM bounty WHERE bounty_on = :ship_id", [
                'ship_id' => $targetinfo['ship_id']
            ]);

            if ($hasbounty) {
                if ($hasbounty['btytotal'] > 0) {
                    $btyamount = NUMBER($hasbounty['btytotal']);
                    $l_scan_bounty = str_replace("[amount]", $btyamount, $l_scan_bounty);
                    echo $l_scan_bounty . "<BR>";
                    $btyamount = 0;
                    
                    // Check for Federation bounty
                    $hasfedbounty = db()->fetch("SELECT SUM(amount) AS btytotal FROM bounty WHERE bounty_on = :ship_id AND placed_by = 0", [
                        'ship_id' => $targetinfo['ship_id']
                    ]);
                    
                    if ($hasfedbounty) {
                        if ($hasfedbounty['btytotal'] > 0) {
            ...
            if ($roll < $success) {
                ...
                echo "<td>" . NUMBER($sc_credits) . "</td></tr>";
            } else {
                echo"<td>???</td></tr>";
            }
