...
$info["P-ONLINE"] = $row[x];
	...
	$res = $db->Execute("SELECT AVG(hull) AS a1 , AVG(engines) AS a2 , AVG(power) AS a3 , AVG(computer) AS a4 , AVG(sensors) AS a5 , AVG(beams) AS a6 , AVG(torp_launchers) AS a7 , AVG(shields) AS a8 , AVG(armour) AS a9 , AVG(cloak) AS a10 FROM ships WHERE ship_destroyed='N' and email LIKE '%@xenobe'");
	$row = $res->fields;
