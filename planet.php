...
/* if there is a planet in the sector show appropriate menu */
    if ($playerinfo['sector'] != $planetinfo['sector_id']) {
        if ($playerinfo['on_planet'] == 'Y') {
            db()->q("UPDATE ships SET on_planet='N' WHERE ship_id= :ship_id", [
    ...
    if ($planetinfo['owner'] != 0) {
        $ownerinfo = db()->fetch("SELECT * FROM ships WHERE ship_id= :owner", [
      ...
      /* ...if there is no planet command already */
        ...
        if ($playerinfo['ship_id'] == $planetinfo['owner']) {
            if ($destroy == 1 && $allow_genesis_destroy) {
                echo "<span style='color: red;'>$l_planet_confirm</span><br><A HREF=planet.php?planet_id=$planet_id&destroy=2>yes</A><br>";
                ...
                if ($playerinfo['dev_genesis'] > 0) {
                    ...
                    db()->q("UPDATE ships SET on_planet='N' WHERE planet_id= :planet_id", [
                        'planet_id' => $planet_id
                    ]);
        ...
        if ($planetinfo['owner'] == $playerinfo['ship_id'] || ($planetinfo['corp'] == $playerinfo['team'] && $playerinfo['team'] > 0)) {
          ...
          /* change production rates */
            ...
            echo "<TR><TH></TH><TH>$l_ore</TH><TH>$l_organics</TH><TH>$l_goods</TH><TH>$l_energy</TH><TH>$l_colonists</TH><TH>$l_credits</TH><TH>$l_fighters</TH><TH>$l_torps</TH></TR>";
            echo "<TR>";
      ...
      /* player owns planet and there is a command */
        if ($command == "sell") {
            if ($planetinfo['sells'] == "Y") {
                ...
                db()->q("UPDATE planets SET sells='N' WHERE planet_id= :planet_id", [
                    'planet_id' => $planet_id
                ]);
            } else {
                echo "$l_planet_nowsell<BR>";
                db()->q("UPDATE planets SET sells='Y' WHERE planet_id= :planet_id", [
                    'planet_id' => $planet_id
                ]);
            }
        } elseif ($command == "name") {
          ...
          /* name2 menu */
            ...
            db()->q("UPDATE planets SET name= :new_name WHERE planet_id= :planet_id", [
                'new_name' => $new_name,
                'planet_id' => $planet_id
            ]);
          ...
          /* land menu */
            ...
            db()->q("UPDATE ships SET on_planet='Y', planet_id= :planet_id WHERE ship_id= :ship_id", [
                'planet_id' => $planet_id,
                'ship_id' => $playerinfo['ship_id']
            ]);
          ...
          /* transfer menu */
            ...
            echo "<TABLE class='table'>";
            echo "<TR><TH>$l_commodity</TH><TH>$l_planet</TH><TH>$l_ship</TH><TH>$l_planet_transfer_link</TH><TH>$l_planet_toplanet</TH><TH>$l_all?</TH></TR>";
            echo "<TR><TD>$l_ore</TD><TD>" . NUMBER($planetinfo['ore']) . "</TD><TD>" . NUMBER($playerinfo['ship_ore']) . "</TD><TD><INPUT TYPE=TEXT NAME=transfer_ore SIZE=10 MAXLENGTH=20></TD><TD><INPUT TYPE=CHECKBOX NAME=tpore VALUE=-1></TD><TD><INPUT TYPE=CHECKBOX NAME=allore VALUE=-1></TD></TR>";
            echo "<TR><TD>$l_organics</TD><TD>" . NUMBER($planetinfo['organics']) . "</TD><TD>" . NUMBER($playerinfo['ship_organics']) . "</TD><TD><INPUT TYPE=TEXT NAME=transfer_organics SIZE=10 MAXLENGTH=20></TD><TD><INPUT TYPE=CHECKBOX NAME=tporganics VALUE=-1></TD><TD><INPUT TYPE=CHECKBOX NAME=allorganics VALUE=-1></TD></TR>";
            echo "<TR><TD>$l_goods</TD><TD>" . NUMBER($planetinfo['goods']) . "</TD><TD>" . NUMBER($playerinfo['ship_goods']) . "</TD><TD><INPUT TYPE=TEXT NAME=transfer_goods SIZE=10 MAXLENGTH=20></TD><TD><INPUT TYPE=CHECKBOX NAME=tpgoods VALUE=-1></TD><TD><INPUT TYPE=CHECKBOX NAME=allgoods VALUE=-1></TD></TR>";
            echo "<TR><TD>$l_energy</TD><TD>" . NUMBER($planetinfo['energy']) . "</TD><TD>" . NUMBER($playerinfo['ship_energy']) . "</TD><TD><INPUT TYPE=TEXT NAME=transfer_energy SIZE=10 MAXLENGTH=20></TD><TD><INPUT TYPE=CHECKBOX NAME=tpenergy VALUE=-1></TD><TD><INPUT TYPE=CHECKBOX NAME=allenergy VALUE=-1></TD></TR>";
            echo "<TR><TD>$l_colonists</TD><TD>" . NUMBER($planetinfo['colonists']) . "</TD><TD>" . NUMBER($playerinfo['ship_colonists']) . "</TD><TD><INPUT TYPE=TEXT NAME=transfer_colonists SIZE=10 MAXLENGTH=20></TD><TD><INPUT TYPE=CHECKBOX NAME=tpcolonists VALUE=-1></TD><TD><INPUT TYPE=CHECKBOX NAME=allcolonists VALUE=-1></TD></TR>";
            echo "<TR><TD>$l_fighters</TD><TD>" . NUMBER($planetinfo['fighters']) . "</TD><TD>" . NUMBER($playerinfo['ship_fighters']) . "</TD><TD><INPUT TYPE=TEXT NAME=transfer_fighters SIZE=10 MAXLENGTH=20></TD><TD><INPUT TYPE=CHECKBOX NAME=tpfighters VALUE=-1></TD><TD><INPUT TYPE=CHECKBOX NAME=allfighters VALUE=-1></TD></TR>";
            echo "<TR><TD>$l_torps</TD><TD>" . NUMBER($planetinfo['torps']) . "</TD><TD>" . NUMBER($playerinfo['torps']) . "</TD><TD><INPUT TYPE=TEXT NAME=transfer_torps SIZE=10 MAXLENGTH=20></TD><TD><INPUT TYPE=CHECKBOX NAME=tptorps VALUE=-1></TD><TD><INPUT TYPE=CHECKBOX NAME=alltorps VALUE=-1></TD></TR>";
            echo "<TR><TD>$l_credits</TD><TD>" . NUMBER($planetinfo['credits']) . "</TD><TD>" . NUMBER($playerinfo['credits']) . "</TD><TD><INPUT TYPE=TEXT NAME=transfer_credits SIZE=10 MAXLENGTH=20></TD><TD><INPUT TYPE=CHECKBOX NAME=tpcredits VALUE=-1></TD><TD><INPUT TYPE=CHECKBOX NAME=allcredits VALUE=-1></TD></TR>";
            echo "</TABLE><BR>";
          ...
          /* change production percentages */
            ...
            if ($porganics < 0.0 || $pore < 0.0 || $pgoods < 0.0 || $penergy < 0.0 || $pfighters < 0.0 || $ptorp < 0.0) {
                ...
                db()->q("UPDATE planets SET prod_ore= :pore, prod_organics= :porganics, prod_goods= :pgoods, prod_energy= :penergy, prod_fighters= :pfighters, prod_torp= :ptorp WHERE planet_id= :planet_id", [
                    'pore' => $pore,
                    'porganics' => $porganics,
                    'pgoods' => $pgoods,
                    'penergy' => $penergy,
      ...
      /* player doesn't own planet and there is a command */
        if ($command == "buy") {
            if ($planetinfo['sells'] == "Y") {
                ...
                echo "<table class='table'>";
                echo "<tr><th>$l_commodity</th><th>$l_avail</th><th>$l_price</th><th>$l_buy</th><th>$l_cargo</th></tr>";
          ...
          /* determine per cent chance of success in scanning target ship - based on player's sensors and opponent's cloak */
            ...
            if ($roll > $success) {
              /* if scan fails - inform both player and target. */
                ...
                playerlog($ownerinfo['ship_id'], \BNT\Log\LogTypeConstants::LOG_PLANET_SCAN_FAIL, sprintf("%s|%s|%s",
                    $planetinfo['name'],
                    $playerinfo['sector'],
                ...
                playerlog($ownerinfo['ship_id'], \BNT\Log\LogTypeConstants::LOG_PLANET_SCAN, sprintf("%s|%s|%s",
                    $planetinfo['name'],
                    $playerinfo['sector'],
                    $playerinfo['character_name']
                ));
              ...
              /* scramble results by scan error factor. */
                ...
                echo "<table class='table'>";
                echo "<tr><th>$l_commodities:</th><th></th></tr>";
                ...
                echo "<tr><th colspan='2'>$l_defense:</th></tr>";
                ...
                $docked_ships = db()->fetchAll("SELECT * FROM ships WHERE on_planet = 'Y' and planet_id = :planet_id", [
                    'planet_id' => $planet_id
                ]);

                foreach ($docked_ships as $row) {
            ...
            db()->q("UPDATE planets SET corp=null, owner= :owner, base='N', defeated='N' WHERE planet_id= :planet_id", [
            ...
            if ($planetinfo['owner'] != 0) {
                $planetowner_query = db()->fetch("SELECT character_name FROM ships WHERE ship_id= :owner", [
                    'owner' => $planetinfo['owner']
                ]);
                $planetowner = $planetowner_query['character_name'];
            } else {
                $planetowner = "$l_planet_noone";
            }

            playerlog($playerinfo['ship_id'], \BNT\Log\LogTypeConstants::LOG_PLANET_CAPTURED, sprintf("%s|%s|%s",
                $planetinfo['colonists'],
                $planetinfo['credits'],
                $planetowner
            ));
        } elseif ($command == "capture" &&  ($planetinfo['owner'] == 0 || $planetinfo['defeated'] == 'Y')) {
            echo "$l_planet_notdef<BR>";
            db()->q("UPDATE planets SET defeated='N' WHERE planet_id= :planet_id", [
