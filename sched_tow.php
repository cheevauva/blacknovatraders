<?
  ...
  do
  {
    $res = $db->Execute("SELECT ship_id,character_name,hull,sector,$dbtables[universe].zone_id,max_hull FROM ships,$dbtables[universe],$dbtables[zones] WHERE sector=sector_id AND $dbtables[universe].zone_id=$dbtables[zones].zone_id AND max_hull<>0 AND ((ships.hull + ships.engines + ships.computer + ships.beams + ships.torp_launchers + ships.shields + ships.armor)/7) >max_hull AND ship_destroyed='N'");
    if($res)
    {
      ...
      while(!$res->EOF)
      {
        ...
        $query = $db->Execute("UPDATE ships SET sector=$newsector,cleared_defences=' ' where ship_id=$row[ship_id]");
