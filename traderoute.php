...
$num_traderoutes = count($traderoutes);
...
if ($playerinfo['ship_colonists'] < 0 || $playerinfo['ship_ore'] < 0 || $playerinfo['ship_organics'] < 0 || $playerinfo['ship_goods'] < 0 || $playerinfo['ship_energy'] < 0 || $freeholds < 0) {
    if ($playerinfo['ship_colonists'] < 0 || $playerinfo['ship_colonists'] > $maxholds) {
        adminlog(LogTypeConstants::LOG_ADMIN_ILLEGVALUE, $playerinfo['ship_id'], "$playerinfo[ship_name]|$playerinfo[ship_colonists]|colonists|$maxholds");
        $playerinfo['ship_colonists'] = 0;
    }
    if ($playerinfo['ship_ore'] < 0 || $playerinfo['ship_ore'] > $maxholds) {
        adminlog(LogTypeConstants::LOG_ADMIN_ILLEGVALUE, $playerinfo['ship_id'], "$playerinfo[ship_name]|$playerinfo[ship_ore]|ore|$maxholds");
        $playerinfo['ship_ore'] = 0;
    }
    if ($playerinfo['ship_organics'] < 0 || $playerinfo['ship_organics'] > $maxholds) {
        adminlog(LogTypeConstants::LOG_ADMIN_ILLEGVALUE, $playerinfo['ship_id'], "$playerinfo[ship_name]|$playerinfo[ship_organics]|organics|$maxholds");
        $playerinfo['ship_organics'] = 0;
    }
    if ($playerinfo['ship_goods'] < 0 || $playerinfo['ship_goods'] > $maxholds) {
        adminlog(LogTypeConstants::LOG_ADMIN_ILLEGVALUE, $playerinfo['ship_id'], "$playerinfo[ship_name]|$playerinfo[ship_goods]|goods|$maxholds");
        $playerinfo['ship_goods'] = 0;
    }
    if ($playerinfo['ship_energy'] < 0 || $playerinfo['ship_energy'] > $maxenergy) {
        adminlog(LogTypeConstants::LOG_ADMIN_ILLEGVALUE, $playerinfo['ship_id'], "$playerinfo[ship_name]|$playerinfo[ship_energy]|energy|$maxenergy");
        $playerinfo['ship_energy'] = 0;
    }
    if ($freeholds < 0) {
        $freeholds = 0;
    }
    db()->q("UPDATE ships SET ship_ore=:ship_ore, ship_organics=:ship_organics, ship_goods=:ship_goods, ship_energy=:ship_energy, ship_colonists=:ship_colonists WHERE ship_id=:ship_id", [
...
if ($num_traderoutes == 0) {
    ...
    while ($i < $num_traderoutes) {
        ...
        if ($traderoutes[$i]['source_type'] == 'P') {
            ...
            $planet1 = db()->fetch("SELECT name, sector_id FROM planets WHERE planet_id=:planet_id", [
                'planet_id' => $traderoutes[$i]['source_id'],
            ]);
            if ($planet1) {
        ...
        if ($traderoutes[$i]['source_type'] == 'P') {
            $port1 = db()->fetch("SELECT * FROM universe WHERE sector_id=:sector_id", [
                'sector_id' => $traderoutes[$i]['source_id'],
            ]);
        ...
        if ($traderoutes[$i]['dest_type'] == 'P') {
            ...
            $planet2 = db()->fetch("SELECT name, sector_id FROM planets WHERE planet_id=:planet_id", [
                'planet_id' => $traderoutes[$i]['dest_id'],
            ]);
            if ($planet2) {
        ...
        echo "<td align='center'><font size=2 color=white>";
        if ($traderoutes[$i]['dest_type'] == 'P') {
            $port2 = db()->fetch("SELECT * FROM universe WHERE sector_id=:sector_id", [
                'sector_id' => $traderoutes[$i]['dest_id'],
            ]);
...
function traderoute_check_compatible($type1, $type2, $move, $circuit, $src, $dest)
{
    ...
    if ($move == 'warp') {
        $link = db()->fetch("SELECT link_id FROM links WHERE link_start=:start AND link_dest=:dest", [
            'start' => $src['sector_id'],
            'dest' => $dest['sector_id'],
        ]);
        if (!$link) {
...
function traderoute_distance($type1, $type2, $start, $dest, $circuit, $sells = 'N')
{
    ...
    if ($type1 == 'L') {
        $start = db()->fetch("SELECT * FROM universe WHERE sector_id=:sector_id", [
            'sector_id' => $start,
        ]);
    }

    if ($type2 == 'L') {
        $dest = db()->fetch("SELECT * FROM universe WHERE sector_id=:sector_id", [
            'sector_id' => $dest,
        ]);
    }
...
function traderoutenew($traderoute_id)
{
    ...
    if (!empty($traderoute_id)) {
        $editroute = db()->fetch("SELECT * FROM traderoutes WHERE traderoute_id=:traderoute_id", [
...
//---- Get Planet info Corp and Personal (BEGIN) ----
    ...
    $planets = db()->fetchAll("SELECT * FROM planets WHERE owner=:owner ORDER BY sector_id", [
        'owner' => $playerinfo['ship_id'],
    ]);
    ...
    $planets_corp = db()->fetchAll("SELECT * FROM planets WHERE corp=:team AND corp!=0 AND owner<>:owner ORDER BY sector_id", [
        'team' => $playerinfo['team'],
        'owner' => $playerinfo['ship_id'],
    ]);
    $num_corp_planets = count($planets_corp);
    foreach ($planets_corp as $i => $planet) {
        if ($planet['name'] == "") {
...
function traderoute_create()
{
    ...
    if ($ptype1 == 'port') {
        $source = db()->fetch("SELECT * FROM universe WHERE sector_id=:sector_id", [
            'sector_id' => $port_id1,
        ]);
        if (!$source) {
        ...
        $source = db()->fetch("SELECT * FROM planets WHERE planet_id=:planet_id", [
            'planet_id' => $planet_id1,
        ]);
        if (!$source) {
...
//Attempting to fix the map the universe via traderoute bug -- rjordan01
    ...
    $pl1 = db()->fetch("SELECT * FROM movement_log WHERE sector_id=:sector_id AND ship_id = :ship_id", [
...
//note: shouldnt we, more realistically, require a ship to be *IN* the source sector to create the traderoute?
    ...
    if ($ptype2 == 'port') {
        $destination = db()->fetch("SELECT * FROM universe WHERE sector_id=:sector_id", [
            'sector_id' => $port_id2,
        ]);
        if (!$destination) {
        ...
        $destination = db()->fetch("SELECT * FROM planets WHERE planet_id=:planet_id", [
...
//OK now we have $destination lets see if we've been there.
    $pl2 = db()->fetch("SELECT * FROM movement_log WHERE sector_id=:sector_id AND ship_id = :ship_id", [
...
//check destination- we cannot trade INTO a special port
    ...
    if (empty($editing)) {
        db()->q("INSERT INTO traderoutes VALUES(NULL, :src_id, :dest_id, :src_type, :dest_type, :mtype, :owner, :circuit)", [
            'src_id' => $src_id,
            'dest_id' => $dest_id,
            'src_type' => $src_type,
            'dest_type' => $dest_type,
        ...
        db()->q("UPDATE traderoutes SET source_id=:src_id, dest_id=:dest_id, source_type=:src_type, dest_type=:dest_type, move_type=:mtype, owner=:owner, circuit=:circuit WHERE traderoute_id=:editing", [
            'src_id' => $src_id,
            'dest_id' => $dest_id,
            'src_type' => $src_type,
            'dest_type' => $dest_type,
...
function traderoute_delete()
{
    ...
    $delroute = db()->fetch("SELECT * FROM traderoutes WHERE traderoute_id=:traderoute_id", [
        'traderoute_id' => $traderoute_id,
    ]);
    if (!$delroute) {
    ...
    if (empty($confirm)) {
        ...
        db()->q("DELETE FROM traderoutes WHERE traderoute_id=:traderoute_id", [
            'traderoute_id' => $traderoute_id,
        ]);
...
function traderoute_setsettings()
{
    ...
    db()->q("UPDATE ships SET trade_colonists=:colonists, trade_fighters=:fighters, trade_torps=:torps, trade_energy=:energy WHERE ship_id=:ship_id", [
        'colonists' => $colonists,
        'fighters' => $fighters,
        'torps' => $torps,
        'energy' => $energy,
...
// ********************************
    if ($traderoute['source_type'] == 'P') {
        ...
        $source = db()->fetch("SELECT * FROM universe WHERE sector_id=:source_id", [
        ...
        $source = db()->fetch("SELECT * FROM planets WHERE planet_id=:source_id", [
            'source_id' => $traderoute['source_id'],
        ]);
        if (!$source) {
        ...
        $sourceport = db()->fetch("SELECT * FROM universe WHERE sector_id=:sector_id", [
            'sector_id' => $source['sector_id'],
        ]);
        if (!$sourceport) {
...
// ********************************
    if ($traderoute['dest_type'] == 'P') {
        $dest = db()->fetch("SELECT * FROM universe WHERE sector_id=:dest_id", [
        ...
        $dest = db()->fetch("SELECT * FROM planets WHERE planet_id=:dest_id", [
            'dest_id' => $traderoute['dest_id'],
        ]);
        if (!$dest) {
        ...
        $destport = db()->fetch("SELECT * FROM universe WHERE sector_id=:sector_id", [
...
// ********************************
    ...
    $fighters_owner = db()->fetch("SELECT * FROM sector_defence WHERE sector_id = :sector_id AND ship_id <> :ship_id", [
        'sector_id' => $source['sector_id'],
        'ship_id' => $playerinfo['ship_id'],
    ]);
    if ($fighters_owner) {
        $nsfighters = db()->fetch("SELECT * from ships where ship_id=:ship_id", [
...
// *********************************************
    if ($traderoute['source_type'] == 'P') {
        $zoneinfo = db()->fetch("SELECT * FROM zones,universe WHERE universe.sector_id=:sector_id AND zones.zone_id=universe.zone_id", [
            'sector_id' => $traderoute['source_id'],
        ]);
        if ($zoneinfo['allow_trade'] == 'N') {
            traderoute_die($l_tdr_nosrcporttrade);
        } elseif ($zoneinfo['allow_trade'] == 'L') {
            if ($zoneinfo['corp_zone'] == 'N') {
                $ownerinfo = db()->fetch("SELECT team FROM ships WHERE ship_id=:ship_id", [
                    'ship_id' => $zoneinfo['owner'],
                ]);
    ...
    if ($traderoute['source_type'] == 'P') {
        ...
        if ($source['port_type'] == 'special') {
            ...
            if ($traderoute['circuit'] == '1') {
                db()->q("UPDATE ships SET ship_colonists=ship_colonists+:colonists_buy, ship_fighters=ship_fighters+:fighters_buy,torps=torps+:torps_buy, ship_energy=ship_energy+:scooped WHERE ship_id=:ship_id", [
                    'colonists_buy' => $colonists_buy,
                    'fighters_buy' => $fighters_buy,
                    'torps_buy' => $torps_buy,
                    'scooped' => $dist['scooped1'],
                    'ship_id' => $playerinfo['ship_id'],
                ]);
            }
        }
        ...
        //-------- Normal Port Section (begin) ------
            ...
            if ($source['port_type'] == 'ore') {
                ...
                $playerinfo['ship_ore'] += $ore_buy;
                $sourcecost -= $ore_buy * $ore_price1;
                db()->q("UPDATE universe SET port_ore=port_ore-:ore_buy, port_energy=port_energy-:energy_buy, port_goods=port_goods-:goods_buy, port_organics=port_organics-:organics_buy WHERE sector_id=:sector_id", [
                    'ore_buy' => $ore_buy,
            ...
            if ($traderoute['circuit'] == '1') {
                db()->q("UPDATE ships SET ship_ore=:ship_ore, ship_goods=:ship_goods, ship_organics=:ship_organics, ship_energy=:ship_energy WHERE ship_id=:ship_id", [
                    'ship_ore' => $playerinfo['ship_ore'],
                    'ship_goods' => $playerinfo['ship_goods'],
                    'ship_organics' => $playerinfo['ship_organics'],
                    'ship_energy' => $playerinfo['ship_energy'],
...
//------------- Source is planet (begin) -----
    elseif (($traderoute['source_type'] == 'L') || ($traderoute['source_type'] == 'C')) {
        ...
        if ($traderoute['dest_type'] == 'P') {
            ...
            if (($playerinfo['ship_id'] == $source['owner']) || ($playerinfo['team'] == $source['corp'])) {
                ...
                if ($traderoute['circuit'] == '1') {
                    db()->q("UPDATE ships SET ship_ore=:ship_ore, ship_goods=:ship_goods, ship_organics=:ship_organics WHERE ship_id=:ship_id", [
                        'ship_ore' => $playerinfo['ship_ore'],
                        'ship_goods' => $playerinfo['ship_goods'],
                        'ship_organics' => $playerinfo['ship_organics'],
                        'ship_id' => $playerinfo['ship_id'],
            ...
            db()->q("UPDATE planets SET ore=ore-:ore_buy, goods=goods-:goods_buy, organics=organics-:organics_buy WHERE planet_id=:planet_id", [
                'ore_buy' => $ore_buy,
                'goods_buy' => $goods_buy,
                'organics_buy' => $organics_buy,
        ...
        elseif (($traderoute['dest_type'] == 'L') || ($traderoute['dest_type'] == 'C')) {
            ...
            if ($traderoute['circuit'] == '1') {
                db()->q("UPDATE ships SET torps=:torps, ship_fighters=:ship_fighters, ship_colonists=:ship_colonists WHERE ship_id=:ship_id", [
            ...
            db()->q("UPDATE planets SET colonists=colonists-:colonists_buy, torps=torps-:torps_buy, fighters=fighters-:fighters_buy WHERE planet_id=:planet_id", [
                'colonists_buy' => $colonists_buy,
                'torps_buy' => $torps_buy,
                'fighters_buy' => $fighters_buy,
                'planet_id' => $source['planet_id'],
    ...
    if ($traderoute['circuit'] == '2') {
        ...
        if ($traderoute['dest_type'] == 'P') {
            ...
            db()->q("UPDATE planets SET colonists=colonists+:colonists_buy, fighters=fighters+:fighters_buy, torps=torps+:torps_buy WHERE planet_id=:planet_id", [
                'colonists_buy' => $colonists_buy,
                'fighters_buy' => $fighters_buy,
                'torps_buy' => $torps_buy,
            ...
            if ($traderoute['source_type'] == 'L' || $traderoute['source_type'] == 'C') {
                db()->q("UPDATE ships SET ship_colonists=:col_dump, ship_fighters=:fight_dump, torps=:torps_dump, ship_energy=ship_energy+:scooped WHERE ship_id=:ship_id", [
                    'col_dump' => $col_dump,
                ...
                if ($setcol == 1) {
                    db()->q("UPDATE ships SET ship_colonists=:col_dump, ship_fighters=ship_fighters-:fight_dump, torps=torps-:torps_dump, ship_energy=ship_energy+:scooped WHERE ship_id=:ship_id", [
                        'col_dump' => $col_dump,
                        'fight_dump' => $fight_dump,
                        'torps_dump' => $torps_dump,
                        'scooped' => $dist['scooped'],
                    ...
                    db()->q("UPDATE ships SET ship_colonists=ship_colonists-:col_dump, ship_fighters=ship_fighters-:fight_dump, torps=torps-:torps_dump, ship_energy=ship_energy+:scooped WHERE ship_id=:ship_id", [
                        'col_dump' => $col_dump,
                        'fight_dump' => $fight_dump,
                        'torps_dump' => $torps_dump,
                        'scooped' => $dist['scooped'],
    ...
    db()->q("UPDATE ships SET turns=turns-:triptime, credits=credits+:total_profit, turns_used=turns_used+:triptime, sector=:newsec WHERE ship_id=:ship_id", [
        'triptime' => $dist['triptime'],
