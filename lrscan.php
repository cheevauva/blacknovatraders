...
if ($sector == "*") {
  ...
  // get sectors which can be reached from the player's current sector
    $result = $db->adoExecute("SELECT * FROM links WHERE link_start='$playerinfo[sector]' ORDER BY link_dest");
    ...
    while (!$result->EOF) {
      ...
      // get number of sectors which can be reached from scanned sector
        $result2 = $db->adoExecute("SELECT COUNT(*) AS count FROM links WHERE link_start='$row[link_dest]'");
        $row2 = $result2->fields;
     ...
     // get port type and discover the presence of a planet in scanned sector
        $result2 = $db->adoExecute("SELECT * FROM universe WHERE sector_id='$row[link_dest]'");
        $result3 = $db->adoExecute("SELECT planet_id FROM planets WHERE sector_id='$row[link_dest]'");
        $resultSDa = $db->adoExecute("SELECT SUM(quantity) as mines from sector_defence WHERE sector_id='$row[link_dest]' and defence_type='M'");
        $resultSDb = $db->adoExecute("SELECT SUM(quantity) as fighters from sector_defence WHERE sector_id='$row[link_dest]' and defence_type='F'");
        ...
        if ($playerinfo['dev_lssd'] == 'Y') {
            $resx = $db->adoExecute("SELECT * from movement_log WHERE ship_id <> $playerinfo[ship_id] AND sector_id = $row[link_dest] ORDER BY time DESC LIMIT 1");
            if (!$resx) {
                echo "<TD>None</TD>";
            } else {
                $myrow = $resx->fields;
  ...
  // get sectors which can be reached through scanned sector
    $result3 = $db->adoExecute("SELECT link_dest FROM links WHERE link_start='$sector' ORDER BY link_dest ASC");
  ...
  // get sectors which can be reached from the player's current sector
    $result3a = $db->adoExecute("SELECT link_dest FROM links WHERE link_start='$playerinfo[sector]'");
    ...
    echo "<TR><TD>";
    $query = $db->adoExecute("SELECT name, owner FROM planets WHERE sector_id=$sectorinfo[sector_id]");
    ...
    $resultSDa = $db->adoExecute("SELECT SUM(quantity) as mines from sector_defence WHERE sector_id='$sector' and defence_type='M'");
    $resultSDb = $db->adoExecute("SELECT SUM(quantity) as fighters from sector_defence WHERE sector_id='$sector' and defence_type='F'");
    $defM = $resultSDa->fields;
    $defF = $resultSDb->fields;
    ...
    if ($playerinfo['dev_lssd'] == 'Y') {
        ...
        echo "<TR><TD>";
        $resx = $db->adoExecute("SELECT * from movement_log WHERE ship_id <> $playerinfo[ship_id] AND sector_id = $sector ORDER BY time DESC LIMIT 1");
        if (!$resx) {
            echo "None";
        } else {
            $myrow = $resx->fields;
