...
$result3 = $db->adoExecute("SELECT * FROM sector_defence WHERE defence_id=$defence_id ");
...
switch ($response) {
    ...
    case "retrieve":
        ...
        if ($quantity > 0) {
            $db->adoExecute("UPDATE sector_defence SET quantity=quantity - $quantity WHERE defence_id = $defence_id");
            if ($defenceinfo['defence_type'] == 'M') {
