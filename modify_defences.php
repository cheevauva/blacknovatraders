...
else
{
   ...
   $resulta = $db->Execute ("SELECT * FROM ships WHERE ship_id = $defence_ship_id ");
...
switch($response) {
   case "fight":
      ...
      else
      {
         ...
         $update4b = $db->Execute ("UPDATE ships SET ship_energy=ship_energy-$playerbeams WHERE ship_id=$playerinfo[ship_id]");
   ...
   case "retrieve":
      ...
      if($quantity > 0)
      {
         ...
         if($defenceinfo['defence_type'] == 'M')
         {
            $db->Execute("UPDATE ships set torps=torps + $quantity where ship_id = $ship_id");
         }
         else
         {
            $db->Execute("UPDATE ships set ship_fighters=ship_fighters + $quantity where ship_id = $ship_id");
         }
         $db->Execute("DELETE FROM $dbtables[sector_defence] WHERE quantity <= 0");
      }
      $stamp = date("Y-m-d H-i-s");

      $db->Execute("UPDATE ships SET last_login='$stamp',turns=turns-1, turns_used=turns_used+1, sector=$playerinfo[sector] where ship_id=$playerinfo[ship_id]");
      bigtitle();
