...
$result = $db->adoExecute("SELECT * FROM ibank_accounts WHERE ship_id=$playerinfo[ship_id]");
...
function IGB_transfer()
{
    ...
    $res = $db->adoExecute("SELECT character_name, ship_id FROM ships WHERE email not like '%@xenobe' ORDER BY character_name ASC");
    while (!$res->EOF) {
        $ships[] = $res->fields;
        $res->MoveNext();
    }

    $res = $db->adoExecute("SELECT name, planet_id, sector_id FROM planets WHERE owner=$playerinfo[ship_id] ORDER BY sector_id ASC");
    while (!$res->EOF) {
        $planets[] = $res->fields;
        $res->MoveNext();
    }
...
function IGB_transfer2()
{
    ...
    if (isset($ship_id)) { //ship transfer
        $res = $db->adoExecute("SELECT * FROM ships WHERE ship_id=$ship_id");
        ...
        if ($IGB_trate > 0) {
            ...
            $res = $db->adoExecute("SELECT UNIX_TIMESTAMP(time) as time FROM IGB_transfers WHERE UNIX_TIMESTAMP(time) > $curtime AND source_id=$playerinfo[ship_id] AND dest_id=$target[ship_id]");
            if (!$res->EOF) {
                $time = $res->fields;
        ...
        $res = $db->adoExecute("SELECT name, credits, owner, sector_id FROM planets WHERE planet_id=$splanet_id");
        if (!$res || $res->EOF) {
        ...
        $res = $db->adoExecute("SELECT name, credits, owner, sector_id, base FROM planets WHERE planet_id=$dplanet_id");
        if (!$res || $res->EOF) {
            IGB_error($l_igb_errunknownplanet, "igb.php?command=transfer");
        }
        $dest = $res->fields;
...
function IGB_transfer3()
{
    ...
    //Need to check again to prevent cheating by manual posts
        ...
        $db->adoExecute("UPDATE ibank_accounts SET balance=balance-$amount WHERE ship_id=$playerinfo[ship_id]");
        $db->adoExecute("UPDATE ibank_accounts SET balance=balance+$transfer WHERE ship_id=$target[ship_id]");

        $db->adoExecute("INSERT INTO IGB_transfers VALUES(NULL, $playerinfo[ship_id], $target[ship_id], NOW())");
        echo $db->ErrorMsg();
    } else {
        if ($splanet_id == $dplanet_id) {
            IGB_error($l_igb_errplanetsrcanddest, "igb.php?command=transfer");
        ...
        $res = $db->adoExecute("SELECT name, credits, owner, sector_id FROM planets WHERE planet_id=$dplanet_id");
        if (!$res || $res->EOF) {
            IGB_error($l_igb_errunknownplanet, "igb.php?command=transfer");
        }
        $dest = $res->fields;
        ...
        $db->adoExecute("UPDATE planets SET credits=credits-$amount WHERE planet_id=$splanet_id");
        $db->adoExecute("UPDATE planets SET credits=credits+$transfer WHERE planet_id=$dplanet_id");
    }
}
...
function IGB_deposit2()
{
    ...
    $db->adoExecute("UPDATE ibank_accounts SET balance=balance+$amount WHERE ship_id=$playerinfo[ship_id]");
    $db->adoExecute("UPDATE ships SET credits=credits-$amount WHERE ship_id=$playerinfo[ship_id]");
}
...
function IGB_loans()
{
    ...
    if ($account[loan] != 0) {
        $curtime = time();
        $res = $db->adoExecute("SELECT UNIX_TIMESTAMP(loantime) as time FROM ibank_accounts WHERE ship_id=$playerinfo[ship_id]");
        if (!$res->EOF) {
            $time = $res->fields;
        }
...
function IGB_borrow()
{
    ...
    $db->adoExecute("UPDATE ibank_accounts SET loan=$amount3, loantime=NOW() WHERE ship_id=$playerinfo[ship_id]");
    $db->adoExecute("UPDATE ships SET credits=credits+$amount WHERE ship_id=$playerinfo[ship_id]");
}
...
function IGB_repay()
{
    ...
    $db->adoExecute("UPDATE ibank_accounts SET loan=loan-$amount,loantime='$account[loantime]' WHERE ship_id=$playerinfo[ship_id]");
    $db->adoExecute("UPDATE ships SET credits=credits-$amount WHERE ship_id=$playerinfo[ship_id]");
}
...
function IGB_consolidate2()
{
    ...
    $query = "SELECT SUM(credits) as total, COUNT(*) as count from planets WHERE owner=$playerinfo[ship_id] AND credits != 0";
...
function IGB_consolidate3()
{
    ...
    $query = "UPDATE planets SET credits=0 WHERE owner=$playerinfo[ship_id] AND credits != 0";
    ...
    $res = $db->adoExecute($query);
    $res = $db->adoExecute("UPDATE planets SET credits=credits + $transfer WHERE planet_id=$dplanet_id");
    $res = $db->adoExecute("UPDATE ships SET turns=turns - $tcost WHERE ship_id = $playerinfo[ship_id]");
}
