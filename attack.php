...
//-------------------------------------------------------------------------------------------------
 $db->Execute("LOCK TABLES ships WRITE, $dbtables[universe] WRITE, $dbtables[bounty] WRITE $dbtables[zones] READ, $dbtables[planets] WRITE, $dbtables[news] WRITE, $dbtables[logs] WRITE");
$result = $db->Execute ("SELECT * FROM ships WHERE email='$username'");
$playerinfo=$result->fields;

$ship_id = stripnum($ship_id);

$result2 = $db->Execute ("SELECT * FROM ships WHERE ship_id='$ship_id'");
...
else
{
  ...
  elseif($flee < $roll2)
  {
    ...
    $db->Execute("UPDATE ships SET turns=turns-1,turns_used=turns_used+1 WHERE ship_id=$playerinfo[ship_id]");
  ...
  else
  {
    ...
    if($targetinfo['dev_emerwarp'] > 0 && $random_value > $chance)
    {
      ...
      $db->Execute("UPDATE ships SET turns=turns-1,turns_used=turns_used+1,rating=rating-$rating_change WHERE ship_id=$playerinfo[ship_id]");
      ...
      $result_warp = $db->Execute ("UPDATE ships SET sector=$dest_sector, dev_emerwarp=dev_emerwarp-1,cleared_defences=' ' WHERE ship_id=$targetinfo[ship_id]");
    ...
    echo "$l_att_beams<BR>";
      ...
      if($targetarmor < 1)
      {
        ...
        if($playerarmor > 0)
        {
          ...
          echo $l_att_ysalv;
          $update3 = $db->Execute ("UPDATE ships SET ship_ore=ship_ore+$salv_ore, ship_organics=ship_organics+$salv_organics, ship_goods=ship_goods+$salv_goods, credits=credits+$ship_salvage WHERE ship_id=$playerinfo[ship_id]");
          $armor_lost=$playerinfo['armor_pts']-$playerarmor;
          $fighters_lost=$playerinfo['ship_fighters']-$playerfighters;
          $energy=$playerinfo['ship_energy'];
          $update3b = $db->Execute ("UPDATE ships SET ship_energy=$energy,ship_fighters=ship_fighters-$fighters_lost, armor_pts=armor_pts-$armor_lost, torps=torps-$playertorpnum, turns=turns-1, turns_used=turns_used+1, rating=rating-$rating_change WHERE ship_id=$playerinfo[ship_id]");
      ...
      else
      {
       $l_att_stilship=str_replace("[name]",$targetinfo['character_name'],$l_att_stilship);
        ...
        $update4 = $db->Execute ("UPDATE ships SET ship_energy=$energy,ship_fighters=ship_fighters-$fighters_lost, armor_pts=armor_pts-$armor_lost, torps=torps-$targettorpnum WHERE ship_id=$targetinfo[ship_id]");
        $armor_lost=$playerinfo['armor_pts']-$playerarmor;
        $fighters_lost=$playerinfo['ship_fighters']-$playerfighters;
        $energy=$playerinfo['ship_energy'];
        $update4b = $db->Execute ("UPDATE ships SET ship_energy=$energy,ship_fighters=ship_fighters-$fighters_lost, armor_pts=armor_pts-$armor_lost, torps=torps-$playertorpnum, turns=turns-1, turns_used=turns_used+1, rating=rating-$rating_change WHERE ship_id=$playerinfo[ship_id]");
        echo "$l_att_ylost $armor_lost $l_armorpts, $fighters_lost $l_fighters, $l_att_andused $playertorpnum $l_torps.<BR><BR>";
      }
      if($playerarmor < 1)
      {
        ...
        if($targetarmor > 0)
        {
          ...
          echo "$l_att_salv<BR>";
          $update6 = $db->Execute ("UPDATE ships SET credits=credits+$ship_salvage, ship_ore=ship_ore+$salv_ore, ship_organics=ship_organics+$salv_organics, ship_goods=ship_goods+$salv_goods WHERE ship_id=$targetinfo[ship_id]");
          $armor_lost=$targetinfo[armor_pts]-$targetarmor;
          $fighters_lost=$targetinfo[ship_fighters]-$targetfighters;
          $energy=$targetinfo[ship_energy];
          $update6b = $db->Execute ("UPDATE ships SET ship_energy=$energy,ship_fighters=ship_fighters-$fighters_lost, armor_pts=armor_pts-$armor_lost, torps=torps-$targettorpnum WHERE ship_id=$targetinfo[ship_id]");
        }
      }
    }
  }
