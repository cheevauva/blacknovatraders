<?
	...
	$result2 = $db->Execute("SELECT * from ships where ship_id=$fm_owner");
        ...
        if ($fighters_owner[team] != $playerinfo[team] || $playerinfo[team]==0)
        {
           switch($response) {
              ...
              case "retreat":
                 ...
                 $db->Execute("UPDATE ships SET last_login='$stamp',turns=turns-2, turns_used=turns_used+2, sector=$playerinfo[sector] where ship_id=$playerinfo[ship_id]");
                 bigtitle();
              ...
              case "pay":
                 ...
                 if($playerinfo[credits] < $fighterstoll)
                 {
                    ...
                    // undo the move
                    $db->Execute("UPDATE ships SET sector=$playerinfo[sector] where ship_id=$playerinfo[ship_id]");
                    $ok=0;
                 }
                 else
                 {
                    ...
                    $db->Execute("UPDATE ships SET credits=credits-$fighterstoll where ship_id=$playerinfo[ship_id]");
              ...
              default:
                 ...
                 $db->Execute("UPDATE ships SET cleared_defences = '$interface_string' WHERE ship_id = $playerinfo[ship_id]");
                 $fighterstoll = $total_sector_fighters * $fighter_price * 0.6;
                 bigtitle();
