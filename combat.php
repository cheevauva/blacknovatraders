...
function calcplanetbeams()
{
    ...
    $res = $db->Execute("SELECT * FROM ships WHERE planet_id=$planetinfo[planet_id] AND on_planet='Y'");
    while(!$res->EOF)
    {
...
//$debug = true;
    ...
    $res = $db->Execute("LOCK TABLES ships WRITE, $dbtables[planets] WRITE");
...
playerlog($ownerinfo[ship_id], LOG_PLANET_BOMBED, "$planetinfo[name]|$playerinfo[sector]|$playerinfo[character_name]|$beamsused|$planettorps|$planetfighterslost");
    ...
    $res = $db->Execute("UPDATE ships SET turns=turns-1, turns_used=turns_used+1, ship_fighters=ship_fighters-$attackerfighters WHERE ship_id=$playerinfo[ship_id]");
...
function shiptoship($ship_id)
{
    ...
    $db->Execute("LOCK TABLES ships WRITE, $dbtables[universe] WRITE, $dbtables[zones] READ");
    ...
    else
    {
        ...
        $update4 = $db->Execute ("UPDATE ships SET ship_energy=$target_energy,ship_fighters=ship_fighters-$target_fighters_lost, armor_pts=armor_pts-$target_armor_lost, torps=torps-$targettorpnum WHERE ship_id=$targetinfo[ship_id]");
    }
