  ...
  else
  {
    ...
    if($module == "useredit")
    {
      ...
      if(empty($user))
      {
        ...
        $res = $db->Execute("SELECT ship_id,character_name FROM ships ORDER BY character_name");
        while(!$res->EOF)
        {
          $row=$res->fields;
      ...
      else
      {
        if(empty($operation))
        {
          $res = $db->Execute("SELECT * FROM ships WHERE ship_id=$user");
          $row = $res->fields;
      ...
      else
      {
        if(empty($operation))
        {
          ...
          echo "<TR><TD><tt>          Planet Owner</tt></TD><TD>";
                                      ...
                                      $ressuba = $db->Execute("SELECT ship_id,character_name FROM ships ORDER BY character_name");
    ...
    elseif($module == "ipedit")
    {
      ...
      if(empty($command))
      {
        ...
        else
        {
          ...
          foreach($bans as $ban)
          {
            ...
            $res = $db->Execute("SELECT character_name, ship_id, email FROM ships WHERE ip_address LIKE '$ban'");
            unset($players);
            while(!$res->EOF)
            {
              $players[] = $res->fields;
      ...
      {
        $res = $db->Execute("SELECT DISTINCT ip_address FROM ships");
        while(!$res->EOF)
        {
        ...
        foreach($ips as $ip)
        {
          ...
          $res = $db->Execute("SELECT character_name, ship_id, email FROM ships WHERE ip_address='$ip'");
          unset($players);
          while(!$res->EOF)
          {
            $players[] = $res->fields;
      ...
      elseif($command == 'banip2')
      {
        ...
        $res = $db->Execute("SELECT DISTINCT character_name FROM ships, $dbtables[ip_bans] WHERE ip_address LIKE ban_mask");
      ...
      elseif($command == 'unbanip')
      {
        ...
        $res = $db->Execute("SELECT DISTINCT character_name FROM ships WHERE $query_string");
    ...
    elseif($module == "logview")
    {
      ...
      $res = $db->execute("SELECT ship_id, character_name FROM ships ORDER BY character_name ASC");
      while(!$res->EOF)
      {
        $players[] = $res->fields;
        $res->MoveNext();
